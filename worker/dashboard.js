export async function dashboard(request, env) {
  ...
  const [topLinks, byPage, totals, recentRows] = await Promise.all([
    env.DB.prepare(`
      SELECT link_url, link_text, link_type, COUNT(*) as clicks
      FROM click_events
    ...
    env.DB.prepare(`
      SELECT ts, page, link_url, link_text, link_type
      FROM click_events
      ORDER BY id DESC
      LIMIT 20
    `).all(),
...
<nav class="period-nav">
  ${[1,7,14,30,90].map(d =>
    `<a href="?key=${encodeURIComponent(key)}&days=${d}" class="${d === days ? 'active' : ''}">${d === 1 ? 'I dag' : d + ' dagar'}</a>`
  ).join('')}
</nav>
...
<div class="section">
  ...
  <table>
    ...
    <tbody>
      ${(topLinks.results || []).map(r => {
        const maxClicks = (topLinks.results[0]?.clicks) || 1;
        const pct = Math.round((r.clicks / maxClicks) * 100);
        return `<tr>
          <td>
            <span class="pill pill-${r.link_type}">${r.link_type === 'internal' ? 'intern' : 'extern'}</span><br>
            <span class="url" title="${esc(r.link_url)}">${esc(r.link_url)}</span>
          </td>
          <td style="color:#666">${esc(r.link_text || '—')}</td>
          <td class="cnt">${r.clicks}<div class="bar-wrap"><div class="bar" style="width:${pct}%"></div></div></td>
        </tr>`;
      }).join('')}
    </tbody>
  </table>
  ...
  <table>
    ...
    <tbody>
      ${(byPage.results || []).map(r => {
        const maxClicks = (byPage.results[0]?.clicks) || 1;
        const pct = Math.round((r.clicks / maxClicks) * 100);
        return `<tr>
          <td>${esc(r.page)}</td>
          <td class="cnt">${r.clicks}<div class="bar-wrap"><div class="bar" style="width:${pct}%"></div></div></td>
...
<div class="section">
  ...
  <table>
    ...
    <tbody>
      ${(recentRows.results || []).map(r => `<tr>
        <td class="ts">${r.ts}</td>
        <td style="color:#555;font-size:0.8rem">${esc(r.page)}</td>
        <td><span class="url" title="${esc(r.link_url)}">${esc(r.link_text || r.link_url)}</span></td>
        <td><span class="pill pill-${r.link_type}">${r.link_type === 'internal' ? 'intern' : 'extern'}</span></td>
      </tr>`).join('')}
    </tbody>
  </table>
</div>
