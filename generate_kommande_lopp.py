...
<!-- End Google Tag Manager -->
  ...
  <style>
    *, *::before, *::after {{ margin: 0; padding: 0; box-sizing: border-box; }}
    ...
    .nav-links a.active {{ color: #D4715E; }}
...
height="0" width="0" style="display:none;visibility:hidden"></iframe></noscript>
  ...
  <footer>
    <div class="footer-brand">
      <span class="run-clubs-logo"><span class="run">RUN</span><span class="clubs">CLUBS</span><span class="suffix">.se</span></span>
  ...
  <script>
    ...
    function syncURL() {{
      ...
      if (activeDist   !== 'all') params.set('dist', activeDist);
    ...
    if (_p.get('region')) {{
      activeRegion = _p.get('region');
      const pill = document.querySelector(`[data-filter="region"][data-value="${{activeRegion}}"]`);
      if (pill) {{
    ...
    if (_p.get('dist')) {{
      activeDist = _p.get('dist');
      const pill = document.querySelector(`[data-filter="dist"][data-value="${{activeDist}}"]`);
      if (pill) {{
        document.querySelectorAll('[data-filter="dist"]').forEach(b => b.classList.remove('active'));
        pill.classList.add('active');
      }}
    }}
    render();
