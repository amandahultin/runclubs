...
def prepare_events(records: list[dict], club_cities: dict[str, str] | None = None) -> list[dict]:
    ...
    for r in records:
        ...
        loc_matches = any(kw in loc for kw in STOCKHOLM_KEYWORDS)
        if not loc_matches and club_cities:
            club_key = normalize_club_name(r.get("club") or "").lower()
            loc_matches = club_cities.get(club_key) == "Stockholm"
        if not loc_matches:
            continue
...
height="0" width="0" style="display:none;visibility:hidden"></iframe></noscript>
  ...
  <header class="page-header">
    <div class="page-header-inner">
      <div class="breadcrumb"><a href="/">Start</a> → <a href="/events">Events</a> → Stockholm</div>
      <div class="page-header-tag">Stockholms run clubs</div>
      <h1>Kommande<br>Events och Weekly runs</h1>
      <p>Här visas alla kommande running events från Strava och weekly runs från löpargrupper, run clubs, communions och run crews i Stockholm.</p>
    </div>
  </header>
  ...
  <div class="city-nav">
    ...
    <div class="city-nav-buttons">
      <a href="stockholm-running-events" class="city-nav-btn active">
